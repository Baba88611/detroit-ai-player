...
05_viewer/      可视化界面：浏览器里开一局、实时观看 AI 每一步的情境 / 选择 / 思考 / 结果，或回放结果文件
docs/assets/    架构示意图
```
...
不想盯着终端和 JSON？装好依赖后在仓库根目录运行：

```bash
python 05_viewer/serve.py
```

它会在本机起一个只监听 `127.0.0.1` 的小服务器并打开浏览器（默认 http://127.0.0.1:8765 ）。你可以：

- **开一局**：选模型（未配置的会标出缺哪个变量）、人格、难度、语言、单章或整局，点开始。页面会逐步显示 AI 面对的情境、可选项、它选了什么、它的理由、这一步带来的效果，以及章节结局。想先看看界面长什么样，勾上 dry-run 演示即可，不调用模型。
- **回放**：`04_execution/results/` 里已有的结果文件都列在首页，点开即可回放；也可以把任意结果 JSON 拖进页面。
- **上帝视角开关**：关掉后只显示情境、选项与 AI 的理由，效果数值与判定隐藏，更接近被测 AI 的视角。

服务器不引入任何第三方依赖，API key 只由 runner 子进程从 `.env` 读取，不会经过浏览器。从界面开的一局与命令行开的一局写出的结果文件完全一致。没有服务器时（例如部署到 GitHub Pages 的静态页），同一页面会退化为只读回放模式。
...
贡献者用开发依赖复现测试环境（普通用户运行不需要）。提交 Issue 或 Pull Request 前请先阅读 [CONTRIBUTING.zh-CN.md](CONTRIBUTING.zh-CN.md)：
