...
调用失败时重试最多 3 次，间隔指数退避。超过次数后中止当前章节；此前已完成的章节 JSON 和最近一次 campaign 检查点（`status=partial`）会保留，当前失败章节不伪造完整结果。campaign_runner 每完成一章就覆盖写入同一个 campaign 文件，全部完成后再把 `status` 从 `partial` 更新为 `complete`。
