...
- `README.md` 与 `README.zh-CN.md` 是一组；`CONTRIBUTING.md` 与
  `CONTRIBUTING.zh-CN.md` 是一组。
- 对其中一份进行实质性修改时，必须在同一个 PR 中同步更新对应语言版本。
- 两种语言中的命令、路径、配置项、信息隔离要求、密钥安全规则和许可证边界
  必须保持一致。纯拼写或措辞修正可以只修改受影响的语言版本。
